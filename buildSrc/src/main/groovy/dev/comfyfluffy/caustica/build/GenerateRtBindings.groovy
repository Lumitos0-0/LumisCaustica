...
abstract class GenerateRtBindings extends DefaultTask {
    ...
    private static final List<Map> PIPELINES = [
            ...
            [prefix: "FOG", source: "pipelines/fog/apply.comp.slang", resources: [
                    OUTPUT: "outputImage", DEPTH: "depthImage", VOLUME: "fogVolume",
                    HIST_A: "fogHistoryA", HIST_B: "fogHistoryB", MOTION: "motionImage"]],
    ...
    void generate() {
        ...
        PIPELINES.each { spec ->
            ...
            if (spec.prefix.toString().startsWith("OVERLAY_")) {
                ...
                def storageImages = guides + ordinary.findAll { suffix, ignored -> suffix in ["OUTPUT", "FOG_VOLUME"] }
