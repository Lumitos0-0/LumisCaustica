...
public final class RtContext {
    ...
    /**
     * Create a 3D storage image (STORAGE + SAMPLED + TRANSFER_SRC/DST), transitioned to GENERAL. One 3D
     * view serves both storage writes and filtered sampling — the froxel fog volume's bake raygen writes
     * through it and the apply pass samples through it.
     */
    public RtVolume createVolume(int width, int height, int depth, int format, String label) {
        int usage = VK10.VK_IMAGE_USAGE_STORAGE_BIT | VK10.VK_IMAGE_USAGE_SAMPLED_BIT
