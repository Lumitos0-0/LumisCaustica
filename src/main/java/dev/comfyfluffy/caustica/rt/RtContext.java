...
public final class RtContext {
    ...
    /**
     * Create a 3D storage image (STORAGE + SAMPLED + TRANSFER_SRC/DST) already transitioned to GENERAL,
     * for the volumetric froxel cache. R16G16B16A16_SFLOAT keeps single-scatter radiance and extinction
     * in HDR; the cache is only a few megabytes even at render-res/16, so the extra mantissa is cheap
     * and avoids quantised shafts near the sun.
     */
    public RtImage createStorageImage3D(int width, int height, int depth, int format, String label) {
        ...
        requireStorageImageSupport3D(width, height, depth, format, usage, label);
        long image;
        long allocation;
        long view;
        try (MemoryStack stack = MemoryStack.stackPush()) {
    ...
    private void requireStorageImageSupport3D(int width, int height, int depth, int format, int usage,
                                              String label) {
        try (MemoryStack stack = MemoryStack.stackPush()) {
