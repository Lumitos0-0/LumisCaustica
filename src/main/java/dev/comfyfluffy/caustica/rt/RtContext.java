...
public final class RtContext {
    ...
    /**
     * Create a 3D storage image of the given format (STORAGE + SAMPLED + TRANSFER_SRC/DST), transitioned to GENERAL.
     * Used for 3D froxel volumetric fog grids and irradiance volumes.
     */
    public RtImage createStorageImage3D(int width, int height, int depth, int format, String label) {
        ...
        requireStorageImage3DSupport(width, height, depth, format, usage, label);
        long image;
        long allocation;
        long view;
        try (MemoryStack stack = MemoryStack.stackPush()) {
    ...
    private void requireStorageImage3DSupport(int width, int height, int depth, int format, int usage, String label) {
        try (MemoryStack stack = MemoryStack.stackPush()) {
