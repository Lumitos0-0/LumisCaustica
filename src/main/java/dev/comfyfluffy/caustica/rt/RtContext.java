...
public final class RtContext {
    ...
    /**
     * Create a 3D storage/sampled image for camera-frustum volume data. Like 2D storage images it remains
     * in {@code GENERAL}; the caller supplies shader barriers between injection, integration, and sampling.
     */
    public RtImage createStorageVolume(int width, int height, int depth, int format, String label) {
        int usage = VK10.VK_IMAGE_USAGE_STORAGE_BIT | VK10.VK_IMAGE_USAGE_SAMPLED_BIT
    ...
    private void requireStorageVolumeSupport(int width, int height, int depth, int format,
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            VK10.vkGetPhysicalDeviceFormatProperties(vk.getPhysicalDevice(), format, formatProperties);
            int required = VK10.VK_FORMAT_FEATURE_STORAGE_IMAGE_BIT | VK10.VK_FORMAT_FEATURE_SAMPLED_IMAGE_BIT
                    | VK11.VK_FORMAT_FEATURE_TRANSFER_SRC_BIT | VK11.VK_FORMAT_FEATURE_TRANSFER_DST_BIT;
            int supported = formatProperties.optimalTilingFeatures();
            if ((supported & required) != required) {
                throw new UnsupportedOperationException(label + " format " + format
                        + " lacks 3D optimal-tiling features 0x"
                        + Integer.toHexString(required & ~supported));
            }
            ...
            check(result, "vkGetPhysicalDeviceImageFormatProperties(3D)");
            if (width > imageProperties.maxExtent().width()
                    || height > imageProperties.maxExtent().height()
                    || depth > imageProperties.maxExtent().depth()) {
                throw new UnsupportedOperationException(label + " extent " + width + "x" + height + "x" + depth
                        + " exceeds 3D format maximum " + imageProperties.maxExtent().width() + "x"
